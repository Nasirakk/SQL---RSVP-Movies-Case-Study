...
FROM   movie; 
-- Solution
-- ID is a Primary key, So definately it won't accept null values. So  you can remove case condition for id column.
...
-- YEAR WISE NO OF MOVIES
SELECT year,
       Count(title) AS NUMBER_OF_MOVIES
FROM   movie
GROUP  BY year;

-- MONTH WISE NO OF MOVIES
...
-- Use the upper pattren for india condition and USA dont use as other countrys usa words presnet in the considiton
...
WITH movies_with_one_genre
     AS (SELECT movie_id,Count(genre)
         FROM   genre
         GROUP  BY movie_id
...
-- Type your code below:
-- Query to find:
-- 1. Number of movies of each genre that start with the word â€˜Theâ€™ (LIKE operator is used for pattern matching)
...
-- Type your code below:
SELECT country, sum(total_votes) as total_votes FROM movie AS m
...
WITH actor_summary
     AS (SELECT NAME  AS actor_name ,
                ...
                Round(Sum(avg_rating * total_votes) / Sum(total_votes), 2) AS
                   actor_avg_rating
         FROM   (select * from movie where upper(country) like '%INDIA%') AS M
                INNER JOIN ratings AS R ON M.id = R.movie_id
                INNER JOIN (select * from role_mapping where category='actor') AS RM ON M.id = RM.movie_id
...
-- Output: Top actor is Vijay Sethupathi with good actor_avg_rating and  mmovie count is eqaul to 5 followed by remaining actors Fahadh Faasil and Yogi Babu*/
...
(
           ...
           FROM       (select * from movie where upper(country) like '%INDIA%' and upper(languages) LIKE '%HINDI%')        AS m
           INNER JOIN ratings   AS r
           ON         m.id=r.movie_id
           INNER JOIN (select * from role_mapping where category='actress') AS rm
           ON         m.id = rm.movie_id
           INNER JOIN names AS n
           ON         rm.name_id = n.id
           GROUP BY   NAME
...
WHERE genre IN(SELECT genre FROM top3_gen)
)
SELECT *
FROM top_movie5
WHERE movie_rank<=5;
